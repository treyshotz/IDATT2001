...
public class MemberArchive {
	...
	public int newMember(Personals personals, LocalDate date) {
		if (personals == null || personals.getSurname().isEmpty() || personals.getFirstname().isEmpty()
				|| personals.getEPostadr().isEmpty() || date == null)
			throw new IllegalArgumentException("Fakk deg, input er null");
	...
	private BonusMember checkGoldLimit(int memberNo, LocalDate date) {
		...
		if (points > GOLD_LIMIT) {
			return new GoldMember(memberNo, member.getPersonals(), date, points);
		}
	...
	private BonusMember checkGoldLimit(BonusMember member, LocalDate date) {
		...
		if (points > GOLD_LIMIT) {
			return new GoldMember(member.getMemberNo(), member.getPersonals(), date, points);
		}
	...
	public boolean deleteMember(int userNo) {
		BonusMember bonusMember = findMember(userNo);
		if (bonusMember == null) {
			System.out.println("FINNES IKKE");
			return false;
		}
		
		members.remove(bonusMember);
		return true;
	}
	
	@Override
	public String toString() {
		return "tiss" + members.toString();
	}
